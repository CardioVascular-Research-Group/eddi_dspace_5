...
public class ItemTag extends TagSupport
{
    ...
    /** Item to output title from */
    private transient Item dbData;
                        ...
                        else if (isResolver) 
                        {
                            ...
                            else
                            {
                                ...
                                if (foundUrn != null)
                                {
                                	...
                                	{
                                		DSpaceObject dso = HandleManager.resolveToObject(context,values[j].value);

                                		if (dso instanceof Item){
                                			Item dbData=((Item)dso);
                                			Metadatum[] dbTitleMetadata = dbData.getMetadata("dc", "title", null, Item.ANY); 

                                			if (dbTitleMetadata != null){
                                				for (int k = 0; k < dbTitleMetadata.length; k++)
                                				{                                    		
                                					if (dbTitleMetadata[k] != null && dbTitleMetadata[k].value != null)
                                					{
